...
Score (DS) which is defined in Equation 1; where M is the segmented ball region
you obtained from Task 1, and S is the corresponding ground-truth binary ball
mask.
...
representing the number of the image, and y-axis representing the corresponding
DS.
...
[best 5] and [worst 5] (along with the corresponding ball GT mask images) in the
Appendix.
...
For each of the ball patches, calculate four different shape features discussed
in the lectures
...
(0°, 45°, 90°, 135°) for the patches from the three balls, separately for each
of the colour channels (red, green, blue).
...
2. Contrast
3. Correlation
and produce perpatch features by calculating the feature average and range
across the 4 orientations. Select one feature from each of the colour channels
and plot the distribution per ball type.
